import { httpClient, parseJwt } from "../utils/httpClient";
...
export const authService = {
    login: async ({ username, password }: any) => {
        ...
        // If response is null (204), it's unexpected for login but handle it
        if (!response) {
    ...
    getUserById: async (userId: string): Promise<IApplicationUser | null> => {
        ...
        // OData single return might be the object directly
        const data = await response.json();
        return data;
    },
